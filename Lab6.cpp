...
double Func(double* x) {
	...
	return (-(pow(x[0], 2)) -8*x[0]- (pow(x[1], 2))-2*x[1]-4* (pow(x[2], 2)) -112*x[2] -797.8);
}
...
double svn(double *x,double a, double *p){ // Алгоритм Свенна
    double h = 0.05;
    while(1) {
        if (Tochka(x,a-h,p) >= Tochka(x,a,p) && Tochka(x,a,p) <= Tochka(x,a+h,p)){
            break;
        } else if(Tochka(x, a-h, p) >= Tochka(x, a, p) && Tochka(x, a, p) >= Tochka(x, a+h, p)){
            a += h/2.;
        } else if(Tochka(x, a-h, p) < Tochka(x, a, p) && Tochka(x, a, p) < Tochka(x, a+h, p)){
            a -= h/2.;
        } else {
